...
#### PDF Markdown格式渲染 🎨
- 📝 **自动渲染Markdown**：PDF报告现在能正确渲染AI返回的Markdown格式
- 🎯 **支持的格式**：
  - 标题：`#`（一级）、`##`（二级）、`###`（三级）
  - 列表：`-`、`*` 开头的项目符号列表（包括列表项中的粗体）
  - 粗体：`**文本**` 格式
  - 段落：自动处理段落和换行
- 🎨 **样式优化**：不同级别的标题使用不同大小和样式
- 📄 **更清晰的结构**：总结内容层次分明，易于阅读

#### PDF大纲/书签功能 📑
- 📑 **自动书签**：每个频道总结自动生成PDF书签
- 🔖 **快速导航**：在PDF阅读器中通过书签快速跳转到不同频道
- 📍 **精准定位**：书签按"分类 / 频道名"格式显示
- 🎯 **兼容性好**：支持Adobe Reader、macOS预览、Chrome等主流PDF阅读器
...
- 新增字体选项：
  ...
  - Windows: 微软雅黑, 黑体, 宋体
- ⚡ **性能优化**：PDF生成在独立线程中运行，避免阻塞Discord事件循环
- 🔄 **异步处理**：使用 `run_in_executor` 处理长时间运行的PDF生成任务

### 📚 文档更新

- 更新 PDF_GUIDE.md 添加故障排查章节
- 说明字体检测命令的使用方法
- 新增 PDF_GUIDE.md 中Markdown格式支持说明
- 添加Markdown渲染效果示例
- 新增 PDF大纲/书签功能说明
- 添加各种PDF阅读器中查看书签的方法
- 更新PDF功能特点列表
